"""Sample raw-pixel CIFAR-10 images from a DiT checkpoint."""
...
if "checkpoint" not in FLAGS:
    flags.DEFINE_string(
        ...
        "outputs/cifar10_pixel/checkpoints/95000 or "
        "gs://diffjax/models/cifar10_pixel_ccond_tpu_15-08-2026/checkpoints. If a "
        "run/checkpoints directory is provided, the highest numeric checkpoint is "
...
if "output_path" not in FLAGS:
    flags.DEFINE_string("output_path", "./cifar10_samples.png", "Path for the output image grid.")
...
def _save_grid(samples: jax.Array, output_path: str) -> None:
    """Save normalized ``[N, H, W, 3]`` samples as an RGB image grid."""
    pixels = (np.asarray((samples + 1.0).clip(0.0, 2.0) / 2.0) * 255.0).round().astype(np.uint8)
    count, height, width, _ = pixels.shape
...
def main(_):
    """Generate a grid of CIFAR-10 samples from a trained checkpoint."""
    ...
    if FLAGS.class_id < -1 or FLAGS.class_id >= config.model.num_classes:
        raise ValueError("--class_id must be -1 or a valid CIFAR-10 class ID (0-9)")
    ...
    cfg_scale = (
        config.evaluation.get("cfg_scale", 1.5) if FLAGS.cfg_scale < 0 else FLAGS.cfg_scale
    ...
    print(f"Generated {FLAGS.num_samples} CIFAR-10 samples ({mode}) on {jax.devices()[0]}.")
